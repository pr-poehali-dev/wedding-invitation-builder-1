...
export default function AdminPanel() {
  ...
  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setAudioError("Файл слишком большой. Максимум 5 МБ — сожмите mp3 до 96–128 kbps.");
      return;
    }
    ...
    try {
      const base64: string = await new Promise((resolve, reject) => {
        ...
        reader.onprogress = (ev) => {
          if (ev.lengthComputable) setAudioProgress(Math.round((ev.loaded / ev.total) * 40));
        };
      ...
      if (!res.ok) {
        if (res.status === 413) setAudioError("Файл слишком большой. Сожмите mp3 (до ~3 МБ).");
        else setAudioError(`Ошибка сервера (${res.status}). Попробуйте ещё раз.`);
        return;
      }
